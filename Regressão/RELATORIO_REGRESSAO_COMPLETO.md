...
## Apêndice: Estrutura de Arquivos do Pipeline (Organizada)

```
Regressão/
├── pipeline_regressao_melhorado.py          # Pipeline principal (FINAL)
├── RELATORIO_REGRESSAO_COMPLETO.md          # Este relatório
├── README_MELHORIAS_REGRESSAO.md            # Resumo das melhorias
├── dataset/
...
│
├── 2_PREPROCESSAMENTO/                       # Dados preprocessados
...
│       └── feature_names.pkl
│
├── 3_MODELAGEM/                              # Modelos e resultados
│   ├── resultados/
...
│   │   └── erros_extremos.csv
│   └── modelos_ajustados/
│       ├── ols.pkl, ridge.pkl, lasso.pkl
│       ├── random_forest.pkl
│       ├── hist_gradient_boosting.pkl
│       └── stacking.pkl
│
├── 4_INTERPRETABILIDADE/                     # TUDO para o seminário
│   ├── coeficientes_ols.png + .csv
│   ├── coeficientes_ridge_lasso.png + .csv
│   ├── shap_values/
│   │   ├── shap_summary.png
│   │   ├── shap_importancia_global.png
│   │   ├── shap_waterfall_superstar.png    (Stephen Curry)
│   │   ├── shap_waterfall_role_player.png  (Frank Kaminsky)
│   │   ├── shap_waterfall_rookie.png       (Jaden Hardy)
...
│       ├── importancia.png + .csv
│
├── 5_AUDITORIA/                              # Auditoria pós-pipeline
│   ├── auditoria_pos_pipeline_regressao.py
│   └── relatorios/
│       └── (outputs de todas as execuções)
│
└── ANTIGO/                                   # Pipeline original (arquivado)
    └── (pipeline_regressao.py, EDA e modelagem original)
```

---

*Relatório desenvolvido para a disciplina de Mineração de Dados — UFSCar*  
*Pipeline fechado em Maio de 2026 — Auditoria pós-pipeline concluída, VIF controlado, interpretabilidade gerada*
